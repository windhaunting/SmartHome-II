...
public class GwServerInterfaceImplRep implements GatewayAllInterfaces{
	    ...
	    //////////////lab3
	    public static boolean GW_STATE = false;           //the Gateway state, false means crash, true means live	    
	    ...
	    protected GwServerInterfaceImplRep() throws RemoteException {
		super();
	...
	public void register(int type, String name, String ip) throws RemoteException {
		...
		if(Const.TYPE_SENSOR == type)
        {
            if(name.equals(Const.NAME_SENSOR_TEMPERATURE))
            {
                ...
                System.out.println("Registered at Gateway Replication");
             ...
             {
            	 System.out.println("Presence Sensor is registered at Gateway");
       ...
       else if(Const.TYPE_SMART_DEVICE == type)
        {
             ...
             {
            	 System.out.println("Smart outlet is registered at Gateway");
	...
	public void report_state(int device_id, int state) throws RemoteException {
      ...
      //  System.out.println("report_state ' s enter here");
		...
		GwServerInterfaceImplRep tmp = new GwServerInterfaceImplRep();
		 ...
		 String filename = Const.CONFIG_IPS_FILE;
			...
			try {
				if (file.createNewFile()) {
					...
					System.out.println("Gate Way Replica Started");
				}
	...
	public static void startThreadRecieve(){
		...
		MultiThreadReceive mltThRev = new MultiThreadReceive(Const.GATEWAY_SERVER_IP_REP, Const.GATEWAY_PORT_REP);		
		mltThRev.start();
		try {
			mltThRev.join();
		} catch (InterruptedException e) {
            ...
            if(!gatewayAction.equals(""))
            {
	            ...
	            if(!gatewayAction.contains(";"))
	            {
	            	if(gatewayAction.equals(Const.GW_TEMP_ACTION))   //if it's Q(Temp), query temp
	            	{
				        ...
				        {
				            String temp = GwServerInterfaceImplRep.GatewayOutputHash.get(currtime);
				            try{
	            	...
	            	else if(gatewayAction.equals(Const.GW_MOTION_ACTION))		//if it's Q(Motion), query Motion
	            	{
	            		MultiThreadRequest mThreadChangeSt = new MultiThreadRequest(false, storeIpAddressSensorsDevices.get(Const.ID_SENSOR_MOTION),
						 ...
						 if(GatewayOutputHash.containsKey(currtime))
				        {
				            String motion = GwServerInterfaceImplRep.GatewayOutputHash.get(currtime);
	 ...
	 */
		...
		if(winner.equals("GATEWAY")){
			...
			long sum=(long) 0;
		    int n = GwServerInterfaceImplRep.storeTimeStamps.size();
		    // Iterating manually is faster than using an enhanced for loop.
		    for (int i = 0; i < n; i++)
		        sum += GwServerInterfaceImplRep.storeTimeStamps.get(i);
	...
	public void ringAlgorithm(String s) throws RemoteException {
		...
		if(!Initiator){
		//if(GwServerInterfaceImplRep.count==0){
	...
	public void setOffsetTimeVariable(Long time){
			...
			GwServerInterfaceImplRep.offsetValuefromTimeServer= (int) (System.currentTimeMillis()-time);
			System.out.println("The offsetvalue for time is set by Using TimeStamp from Master to :"+ GwServerInterfaceImplRep.offsetValuefromTimeServer );
	}
	...
	public void takeControlOfDevices(int type,String name, String ip_Address) throws RemoteException {
		System.out.println("came into take control"+name+type);
		int id=0;
		GwServerInterfaceImplRep tmp = new GwServerInterfaceImplRep();
		tmp.register(type,name,ip_Address);
		if(name.equals(Const.NAME_DATABASE))
			id=Const.ID_DATABASE;
		else if(name.equals(Const.NAME_DEVICE_BULB))
			id= Const.ID_DEVICE_BULB;
		else if(name.equals(Const.NAME_DEVICE_OUTLET))
			id=Const.ID_DEVICE_OUTLET;
		else if(name.equals(Const.NAME_SENSOR_DOOR))
			id=Const.ID_DOOR;
		else if(name.equals(Const.NAME_SENSOR_MOTION))
			id=Const.ID_SENSOR_MOTION;
		else if(name.equals(Const.NAME_SENSOR_PRESENCE))
			id=Const.ID_SENSOR_PRESENCE;
		else if(name.equals(Const.NAME_SENSOR_TEMPERATURE))
			id= Const.ID_SENSOR_TEMPERATURE;
		/**
		 * Start a new thread and configure the Gate way IP Address and Gateway Port in concerned sensor or device for further communication 
		 */
		System.out.println("came into take control after assign"+id);
		UnicastControllingGatewayIPAddress r = new UnicastControllingGatewayIPAddress(id);
	...
	System.out.println("Fault Tolerant Task starts");
		...
		FutureTask<Boolean> FTDetect = new FutureTask<Boolean>(
				new Callable<Boolean>(){
			        public Boolean call() throws Exception {
			        	...
			        	GwServerInterfaceImplRep.DetectGateWayAlive();        //detect whether gateway crashes
		...
		FutureTask<Boolean> FTHeartBeat = new FutureTask<Boolean>(
				new Callable<Boolean>(){
			        public Boolean call() throws Exception {
			        	Thread.sleep(5000);		//fault tolerant execuation after 5s 
			        	PeriodicalSendHeartBeatToGw();		//send heartbeat message to gateway for it to detect gateway replica
			        	return true;
			        }
				});
		 ...
		 executor.execute(FTHeartBeat);  //uncomment this, we can test gateway replica crashed and how gateway detect and recover
...
public static void PeriodicalSendHeartBeatToGw()
{
	
	Runnable ControlRunnable = new Runnable() {
	    public void run() {
	    	 System.out.println("Enter 1 to cause this replica fault/crash(not sending hearbeat); Enter 0 to recover");
	...
	/*
	 * lookup gateway rmi and send message to gateway 
	 */
	public static void SendHeartBeatMsgToGw()
	{
		//System.out.println("Enter SendHeartBeatMsgToGw aaaaaa");
 ...
 * 
 */
	public static void DetectGateWayAlive()
	{
		Runnable DetectRunnable = new Runnable() {
		    public void run() {
		    	...
		    	if(receiveHeartBeatCountCurrent >= Const.FT_TIMES_NO_HEART)
		    	{
		    		...
		    		GW_STATE = false;
		    		System.out.println(" Detect GateWay  is crashed, needs to take over gateway");

		    		//processRecord is to record which sensor is registered with the gateway
...
/*
 *receive message from gateway, not replica
 * @see com.GatewayInterface.GatewayAllInterfaces#HeartMessage(java.lang.String)
 */
	@Override
	public void HeartMessage(String message) throws RemoteException {
		// TODO Auto-generated method stub
		//deal with message received from gateway 
		//System.out.println("Enter receive gw's HeartMessagett  "+message);

		if(message.equals(Const.FT_LIVE_HEART_MSG))
		{
			System.out.println("receive gateway's HeartBeat Message");
		...
		Thread trd = new Thread(){
		    ...
		    while (iterator.hasNext()){
		   	    ...
		   	    if(!storeIpAddressSensorsDevices.containsKey(id))
		   	    {
		   	    	...
		   	    	System.out.println("ipAddress = " +GwServerInterfaceImplRep.processRecord.get(id));
		    	 	...
		    	 	switch(id)
		    		{
			    		case Const.ID_DEVICE_BULB:
			    			...
			    			catageory="DEVICE";
			    			ipAddress = GwServerInterfaceImplRep.processRecord.get(id);
		    	 	...
		    	 	//notify corresponding's sensors and devices gateway crashed
	    			NotifySensorsDevicesFT(Const.ID_GATEWAY,ipAddress,port,Lookup,catageory);
		   	    }
	    	   }
		    }
		  };
