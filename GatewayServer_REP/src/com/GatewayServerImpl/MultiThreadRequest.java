			 ...
			 case Const.ID_DEVICE_OUTLET: 				//change state
			 {
				 SmartCtrlInterfaces stHeaterObj = null;
					...
					try {
						...
						int res  = (result) ? 1 : 0;
						GwServerInterfaceImplRep.Device_States.put(device_id, res);
								...
								int logicClk = LClockGW.peek();
						         ...
						         if(ClockGWVlaue.containsKey(Const.EVT_GW_MOTION_MOTION_SENSED) && ClockGWVlaue.containsKey(Const.EVT_DOOR_OPEN_SENSED))
						         {
							         if(ClockGWVlaue.get(Const.EVT_GW_MOTION_MOTION_SENSED) < ClockGWVlaue.get(Const.EVT_DOOR_OPEN_SENSED) &&  
							        		 GwServerInterfaceImplRep.Device_States.get(Const.ID_SENSOR_PRESENCE) == Const.ON )
							         {
								         ...
								         t.join();
							        		 ...
							        		 GwServerInterfaceImplRep.Device_States.get(Const.ID_SENSOR_PRESENCE) == Const.OFF )
							         {
			 ...
			 case Const.ID_SENSOR_TEMPERATURE: 				//query state
			 {
				...
				try {
					...
					else if(1 == state_changed)    //for TA's Testcase Function only
					{
		               ...
		               if(GwServerInterfaceImplRep.GatewayOutputHash.containsKey(currenTimeStamp))
		               {
		            	   String motion = GwServerInterfaceImplRep.GatewayOutputHash.get(currenTimeStamp);
		               ...
		               System.out.println(" currenTimeStamp Temp here" + currenTimeStamp + "outValue " + outValue);
		                ...
		                GwServerInterfaceImplRep.GatewayOutputHash.put(currenTimeStamp, outValue);
		                GwServerInterfaceImplRep.GatewayOutputArrayStr.add(currenTimeStamp);
					}
					...
					GwServerInterfaceImplRep.Device_States.put(device_id, tempDegree);
			 ...
			 case Const.ID_SENSOR_MOTION: 			//query state
			 {
				...
				try {
						...
						if(1 == state_changed)   	//for TA's Testcase Function only
						{
					        ...
					        if(GwServerInterfaceImplRep.GatewayOutputHash.containsKey(currenTimeStamp))
					        {
					            String temp = GwServerInterfaceImplRep.GatewayOutputHash.get(currenTimeStamp);
						...
						else if((0 == state_changed) ||(2 == state_changed))
						{
							...
							result = Retmsg.QueryStateResult;
							 ...
							 if(result==Const.ON && GwServerInterfaceImplRep.userMode.equals(Const.USER_MODE_HOME)){
									TimerforMotion.main(LClockGW);
								 }
								 if(result==Const.ON && GwServerInterfaceImplRep.userMode.equals(Const.USER_MODE_AWAY)){
						...
						GwServerInterfaceImplRep.Device_States.put(device_id, result);
		...
		if(tempDegree < Const.TASK1_TEMP_LOW_THRESHOLD)
		{
				...
				try {
					...
					if(notifyResult)
					{
						...
						GwServerInterfaceImplRep.Device_States.put(Const.ID_DEVICE_OUTLET, Const.ON);
		...
		else if(tempDegree > Const.TASK1_TEMP_HIGH_THRESHOLD)
		{
				...
				try {
					...
					if(notifyResult)
					{
						...
						//don't forget to update state
						GwServerInterfaceImplRep.Device_States.put(Const.ID_DEVICE_OUTLET, Const.OFF);
