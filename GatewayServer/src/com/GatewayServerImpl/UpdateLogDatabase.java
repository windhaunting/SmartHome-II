...
public class UpdateLogDatabase implements Runnable {
	...
	public void run() {
		if(GwServerInterfaceImpl.CacheHelperQueue.contains(Integer.parseInt(deviceID))){
			
			GwServerInterfaceImpl.CacheHelperQueue.remove(Integer.parseInt(deviceID));
			GwServerInterfaceImpl.CacheHelperQueue.addFirst(Integer.parseInt(deviceID));
